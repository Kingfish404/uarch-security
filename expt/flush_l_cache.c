...
void evict(char *mem, char *mapping, int level)
{
    ...
    // mmap驱逐代码
    // cache to l2 by mmap
    // char *m = (char *)mmap(0, 8 * 4096, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE, -1, 0);

    // cache to l3 by mmap
    // char *m = (char *)mmap(0, 8 * 8 * 4096, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE, -1, 0);
}
...
void test_cache_visited(int level, int rdtsc_base_time)
{
    ...
    printf("| stats | %13ld | %6ld | %6ld | %6ld | %6ld | %6ld |\n",
           visit_times, sum[0], sum[1], sum[2], sum[3],
           sum[0] + sum[1] + sum[2] + sum[3]);
    printf("| sum_time_stamp | avg maccess |\n");
...
int main(void)
{
    ...
    printf("| test  | maccess times | l1 hit | l2 hit | l3 hit | FB hit | all hit |\n");
    for (int i = 1; i < 5; i++)
    {
